...
test_des-ecb_base64 () {
	mkdir -p $TEST_DIR
	# Decrypt base64
	openssl des-ecb -pbkdf2 -a -k passwd -S 0123456789abcdef -in README.md -out $TEST_DIR/output
	...
	assertEquals "" "$output"
	# Decrypt without \n in file
	openssl des-ecb -pbkdf2 -a -k passwd -S 0123456789abcdef -in Makefile -out $TEST_DIR/output
	...
	openssl des-ecb -d -pbkdf2 -a -k passwd -S 0123456789abcdef -in $TEST_DIR/output -out $TEST_DIR/original
	output=$(diff Makefile $TEST_DIR/original)
	assertEquals "" "$output"
	rm -rf $TEST_DIR
}
...
suite_addTest test_des-ecb_base64
